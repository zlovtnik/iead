...
function User.update_password(user_id, new_password)
  ...
  conn:execute(string.format(
    "UPDATE users SET password_hash = '%s', password_reset_required = false WHERE id = %d",
    password_hash:gsub("'", "''"),
    tonumber(user_id)
  ))
...
function User.increment_failed_attempts(user_id)
  ...
  if result and tonumber(result.failed_login_attempts) >= 5 then
    conn:execute(string.format("UPDATE users SET is_active = false WHERE id = %d", tonumber(user_id)))
  end
...
function User.activate(user_id)
  ...
  conn:execute(string.format(
    "UPDATE users SET is_active = true, failed_login_attempts = 0 WHERE id = %d", 
    tonumber(user_id)
  ))
